...
EBS 스냅샷으로 저장 및 AWS Backup 서비스로 백업 가능
    - **캐싱볼륨**: 기본데이터는 S3 에 저장하고 자주 액세스하는 이부 데이터만 로컬 캐싱에 저장, 캐시를 통해 액세스
...
> 애플리케이션 데이터를 온프레미스에서 **AWS 클라우드로 마이그레이션**해야 하는 경우 사용_
...
> _**[실전]** **S3 호스팅 + CloudFront**: 오리진 S3 버킷에 대한 부하를 줄이고, 글로벌 콘텐츠 제공 애플리케이션 서비스를 제공_
> - _원본 부하를 줄이려면 사실상 **원본에 대한 직접적 액세스를 제한**해야 하므로,_
> - _아래 CloudFront 보안에서 **OAI 접근 제어**를 적용하고 원본과 연계된 CloudFront 의 도메인을 고객에게 제공해야 함_

> _**[실전]** **CloudFront** 배포 목적이 각 지역별 **HTTP 프로토콜의 콘텐츠 제공의 최적화**라면,

...
MVT 와 유사한 MVC 패턴을 먼저 살펴보자면,
**모델(Model)**, **뷰(View)**, **컨트롤러(Controller)** 로 구성되어 있는데,
**모델**은 DB 의 데이터를 다루며 비즈니스 로직을 수행하고 **뷰**는 사용자에게 보여지는 화면과 데이터를 나타내며
**컨트롤러**는 사용자의 요청을 받아 모델과 뷰를 적절하게 다룹니다.

장고에서 MVT 패턴은 MVC 와 유사하게 **모델(Model)**, **뷰(View)**, **템플릿(Template)** 으로 구성되어 있는데
주목할 점은 MVC 의 뷰 역할을 MVT 의 템플릿이, 컨트롤러의 역할을 뷰가 수행한다는 점입니다.

MVC 와 MVT 는 사실상 명칭만 다르지 기능적으로 완전히 동일하다고 봐도 좋습니다.

결론적으로 장고에서 **사용자에게 보여지는 부분은 템플릿**이,
**사용자의 요청을 받아 모델을 이용해 적절한 결과를 반환하는 기능은 뷰**가 수행하게 됩니다.

<br>

이러한 구조에서 주요한 이슈 중 하나는, 프로젝트의 **비즈니스 로직을 어디에 어떻게 구현하는가** 하는 것입니다.

MVT 에서 **모델 부분에 비즈니스 로직을 구현하는 것이 일반적**으로 권장되나,
...
이번 **장고 모델 가이드라인** 번역글은 다년간의 장고 프로젝트 경험을 통해 다양한 구현 사례들을 경험한 저자의 실용적인 노하우입니다.
...
<small>image by
**<a href="https://jairvercosa.medium.com/manger-vs-query-sets-in-django-e9af7ed744e0"
target="_blank">Jair Verçosa's post of medium</a>**</small>

<br>
### 뚱뚱한 모델(Fat Models)이 나쁜 이유
...
**메서드를 단순하게 유지하고, 의존성을 회피하며, 더 간단한 테스트를 누리**십시오.

<br>
**(3) Single Responsibility Principle(SRP) 규칙을 위반함**

뚱뚱한 모델은 특정 클래스가 담당해야 하는 역할을 더 추가하도록 만들 것입니다.
저는 프로젝트에서 사람들이 "Customer" 와 같은 모델이 이메일과 공지사항을 전송하도록 만드는 것을 자주 목격했습니다.

모델이 담당하는 역할은 이메일을 전송하는 것이 아니라 데이터를 다루는 것입니다.
...
위 번역글에서 저자는 이론적 측면에서 모델의 목적에 충실한 모듈을 설명하고 있습니다.
...
저자의 노하우가 원리에 더 부합한 것이 사실이지만, 관건은 모든 결합된 로직을 전부 함수로 분리해야만 하느냐는 것입니다.
